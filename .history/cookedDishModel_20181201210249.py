...
train_dish = [TRAIN_DIR+i for i in os.listdir(TRAIN_DIR) if 'mokka' in i]
train_notDish = [TRAIN_DIR +
                 i for i in os.listdir(TRAIN_DIR) if not 'mokka' in i]
...
dish_avg = np.array([dish[0].T for i, dish in enumerate(
    train) if labels[i] == 1]).mean(axis=0)
plt.imshow(dish_avg)
plt.title('The average cooked dish')
plt.show()

notDish_avg = np.array([notDish[0].T for i, notDish in enumerate(
    train) if labels[i] == 0]).mean(axis=0)
